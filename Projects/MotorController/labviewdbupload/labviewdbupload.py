...
#file = open('C:\\logsController\\MC1618DLS 3ÔÂ-12-20_141254.txt')
f = file.read(-1)

sendarray = []
###headers need to be in this format example: "Time,Vendor_Name,PO,FW"
header = "Time,Inspection_Point,Vendor_Name,Vendor_Number,Inspector,Station,PO,PO_Line,Part_Name,Part_Number,Serial_Number,FW,ILimit,IRcomp,Vout,Vout_Ripple,Vout_Loaded,Vpass,Incline_Up,Incline_Down,Vmicro,VFET,HV_Bus,rCurrent,SP_Mid,SP_Low,SP_High,IRcompM,Current_Limit,Foldback"
    ...
    for i in a:
        if len(sendarray) == 30:
            ...
            if sendarray[29] == '':
                ...
                cursor.execute("INSERT INTO LabVIEW.ControllerTest (" + header + ") VALUES " + str(tuple(sendarray)))
                return (sendarray,a)
        if i != 'FAILED':
            sendarray.append(i)            
        elif i == 'FAILED':
            for j in range(30 - len(sendarray)):
...
if (__name__ == "__main__"):
    ...
    while len(a) >=30:          ###This cycles throught the whole log.txt file
        sendarray = []
        a = pop29(a)
        sendarray,a = Upload(a)
